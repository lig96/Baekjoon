# 방법 1: 경사하강법
# O(N * (EPOCHS*DIM))
# 라인 A에 맞춰서 lr 초기값은 1e4로 설정
# 범위가 -1e6~1e6이라서 커야할 듯
#
# 방법 2: n차원 삼분탐색
# O(N * ((ITER*2)^DIM))
# 여기서 2는 llh, lhh.
...
for _ in range(Z):
    ...
    if MODE == 'gradient descent':
        ...
        if N <= 2:
            for i in range(DIM):
                temp = dots[0][i] + (dots[-1][i]-dots[0][i])*0.5
                print(f'{temp:.20f}', end=' ')
        else:
        ...
        ITER = min(
            500, int((int(1e7)//N)**(1/DIM))//2
        )

        if N <= 2:
            for dim in range(DIM):
                temp = dots[0][dim] + (dots[-1][dim]-dots[0][dim])*0.5
                print(f'{temp:.20f}', end=' ')
        else:
